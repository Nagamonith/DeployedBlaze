...
export class ProjectsDetailsComponent implements OnInit {
  ...
  //   });
  // }
  loadProjectSummary(project: any, startDate?: string | null, endDate?: string | null): void {
  const key = `${project.Project_Name}__${project.Target_Version}`;
  ...
  this.http.get<any[]>(`${this.apiBaseUrl}/api/GanttProjects/bug-time-summary`, { params })
    .subscribe({
      next: (data) => {
        this.projectSummary[key] = data || [];
        this.projectSummaryLoading[key] = false;
...
clearDateFilter() {
  ...
  const project = this.tabs[this.activeTabIndex]?.project;
  if (project) {
    this.loadProjectSummary(project); // reload without filter
  }
}
